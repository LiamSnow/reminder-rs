...
use super::{objects::{generics::*, valarm::VAlarm, vevent::VEvent, vtodo::VTodo}, values::base::{ICalMultiple, ICalOptional, ICalValue, ICalValueType}};
use crate::ical::objects::macros::*;
...
impl ICSAbleWithName for Vec<ICalObject> {
    ///this is just so the macro works
...
macro_rules! all_to_ics {
    ($self:expr, $ics:expr, $($param:ident),+) => {
        $(
            $self.$param.to_ics_with_name(&ical_prop_name!($param), $ics);
        )+
    };
}

impl ICSAble for VTodo {
    fn to_ics(&self, ics: &mut String) {
        begin(ics, VTodo::NAME);

        VTodo::get_field_names();

        // all_to_ics!(self, ics,
        //     uid, dtstamp, class, completed, created, description, dtstart, geo,
        //     last_modified, location, organizer, percent_complete, priority,
        //     recurrence_id, sequence, status, summary, url, due, duration,
        //     attach, attendee, categories, comment, contact,
        //     exdate, request_status, related_to, resources, rdate, rrule,
        //     alarms, unknown
        // );
...
impl ICSAble for VAlarm {
    fn to_ics(&self, ics: &mut String) {
        todo!()
    }
}

impl ICSAble for VEvent {
    fn to_ics(&self, ics: &mut String) {
        todo!()
    }
}
...
fn begin(ics: &mut String, name: &str) {
    ics.push_str(CRLF);
    ics.push_str("BEGIN:");
    ics.push_str(name);
}

fn end(ics: &mut String, name: &str) {
    ics.push_str(CRLF);
    ics.push_str("BEGIN:");
    ics.push_str(name);
}
